...
function translateWoWDescription(text) {
  ...
  const fullSentences = [
    ...
    [/Throw a bomb that sticks to the enemy (?:for|durante) (\d+)\s*s\.\s*At the end of the duration the bomb will explode,\s*dealing (.*?) to the target and (\d+) enemies within (\d+)\s*yds of the target\.\s*Can target corpses\.?/gi, 'Lanza una bomba que se adhiere al enemigo durante $1 s. Al finalizar la duración la bomba explotará, infligiendo $2 al objetivo y a $3 enemigos a menos de $4 m del objetivo. Puede lanzarse sobre cadáveres.'],
  ...
  const phrases = [
    ...
    [/tempo de reutilización/gi, 'tiempo de reutilización'],
    ...
    // Clean up double translations or remaining glitches
  ...
  // Clean trailing spaces and periods
  es = es.replace(/\s+/g, ' ').trim();
  return es;
}

// Test against the sample spells from screenshot
const samples = [
  "Removes the cooldown of Reload.",
  "Teaches you Build: ZIGGI-6K. Build: ZIGGI-6K, the latest in regenerative technology, to aid you in healing allies until dismissed.",
  "Teaches Build: Clockwork Assisant and reduces the cooldown of Shotgun Scrap Shots by 0.5 sec.",
  "Throw a bomb that sticks to the enemy for 5 sec. At the end of the duration the bomb will explode, dealing 17+0+FireP*.407381+AP*0.213 Fire damage to the target and 4 enemies within 8 yds of the target. Can target corpses.",
  "Fire a powerful rocket at an enemy that deals 126+0+RAP*0.35+FireP*.35 Fire damage.",
  "Launch 3 Deathballs in a line, increasing in speed the further they travel. They explode when they come into contact with an enemy, dealing 1026+6.1 Points Per Level+RAP*.35 Fire damage to nearby enemies, increased based on distance traveled. Build abilities reduce the cooldown of this ability by 2 sec."
];

for (const s of samples) {
  console.log('Original EN:', s);
  console.log('Clean ES:   ', translateWoWDescription(s));
  console.log('---');
}
